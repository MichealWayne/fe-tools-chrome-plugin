export default [
  {
    id: 'mail',
    name: '邮箱',
    description: '只允许英文字母、数字、下划线、英文句号、以及中划线组成',
    regexShow: '^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$',
  },
  {
    id: 'phone',
    name: '手机号',
    description: '',
    regexShow: '^1(3|4|5|6|7|8|9)\\d{9}$',
  },
  {
    id: 'tel',
    name: '电话',
    description: '固定电话',
    regexShow: '(\\(\\d{3,4}\\)|\\d{3,4}-|\\s)?\\d{8}',
  },
  {
    id: 'url',
    name: 'URL地址',
    description: '',
    regexShow:
      '^((http://)|(https://))?([a-zA-Z0-9]([a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,6}(/?)',
  },
  {
    id: 'ipv4',
    name: 'IP',
    description: '符合IPV4',
    regexShow:
      '^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$',
  },
  {
    id: 'ipv6',
    name: 'IP',
    description: '符合IPV6',
    regexShow:
      '^(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$',
  },
  {
    id: 'userName',
    name: '用户名',
    description: '字母开头，允许5-16字节，允许字母数字下划线',
    regexShow: '^[a-zA-Z][a-zA-Z0-9_]{4,15}$',
  },
  {
    id: 'chinese',
    name: '中文',
    description: '',
    regexShow: '^[\u4e00-\u9fa5]{1,}$',
  },
  {
    id: 'Integer',
    name: '整数数字',
    description: '',
    regexShow: '^-?[1-9]\\d*$',
  },
  {
    id: 'PositiveInteger',
    name: '正整数数字',
    description: '',
    regexShow: '^[1-9]\\d*$',
  },
  {
    id: 'NegativeInteger',
    name: '负整数数字',
    description: '',
    regexShow: '^-[1-9]\\d*$',
  },
  {
    id: 'number4',
    name: '非负整数数字',
    description: '',
    regexShow: '^[1-9]\\d*|0$',
  },
  {
    id: 'number5',
    name: '非正整数数字',
    description: '',
    regexShow: '^-[1-9]\\d*|0$',
  },
  {
    id: 'Positivenumber',
    name: '零和正数',
    description: '',
    regexShow: '^\\d*\\.?\\d+$',
  },
  {
    id: 'Negativenumber',
    name: '负数',
    description: '',
    regexShow: '^-\\d*\\.?\\d+$',
  },
  {
    id: 'number6',
    name: '浮点数',
    description: '不能为整数',
    regexShow: '^-?([1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*|0?\\.0+|0)$',
  },
  {
    id: 'number7',
    name: '正浮点数',
    description: '不能为整数',
    regexShow: '^[1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*$',
  },
  {
    id: 'number8',
    name: '负浮点数',
    description: '不能为整数',
    regexShow: '^-([1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*)$',
  },
  {
    id: 'number9',
    name: '非负浮点数',
    description: '不能为整数',
    regexShow: '^[1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*|0?\\.0+|0$',
  },
  {
    id: 'number10',
    name: '非正浮点数',
    description: '不能为整数',
    regexShow: '^(-([1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*))|0?\\.0+|0$',
  },
  {
    id: 'englishAndNumber',
    name: '英文和数字',
    description: '',
    regexShow: '^[A-Za-z0-9]+$',
  },
  {
    id: 'englishAndNumberAndLine',
    name: '中文英文数字或下划线',
    description: '由中文、数字、26个英文字母或者下划线组成的字符串',
    regexShow: '^[\u4E00-\u9FA5A-Za-z0-9_]+$',
  },
  {
    id: 'chineseAndEnglishAndNumberAndLine',
    name: '中文英文数字或下划线',
    description: '中文、英文、数字包括下划线',
    regexShow: '^\\w+$',
  },
  {
    id: 'chineseAndEnglishAndNumberNotLine',
    name: '中文英文数字无符号',
    description: '中文、英文、数字但不包括下划线等符号',
    regexShow: '^[\u4E00-\u9FA5A-Za-z0-9]+$',
  },
  {
    id: 'lengthExample',
    name: '长度限制',
    description: '长度为6～8，任意字符',
    regexShow: '^.{6,8}$',
  },
  {
    id: 'english',
    name: '英文',
    description: '由26个英文字母组成的字符串',
    regexShow: '^[A-Za-z]+$',
  },
  {
    id: 'english2',
    name: '英文',
    description: '由26个大写英文字母组成的字符串',
    regexShow: '^[A-Z]+$',
  },
  {
    id: 'english3',
    name: '英文',
    description: '由26个小写英文字母组成的字符串',
    regexShow: '^[a-z]+$',
  },
  {
    id: 'noSymbol1',
    name: '禁止符号',
    description: '禁止输入含有%&",;=?$"等字符',
    regexShow: '[^%&",;=?$\\x22]+',
  },
  {
    id: 'noWave',
    name: '禁止~符号',
    description: '禁止输入含有~的字符',
    regexShow: '[^~\\x22]+',
  },
  {
    id: 'dateFormat',
    name: '日期格式',
    description: '日期格式符合mm/dd/yyyy规范',
    regexShow:
      '^(0?[1-9]|[12][0-9]|3[01])([ /-])(0?[1-9]|1[012])\\2([0-9][0-9][0-9][0-9])(([ -])([0-1]?[0-9]|2[0-3]):[0-5]?[0-9]:[0-5]?[0-9])?$',
  },
  {
    id: 'timeFormat',
    name: '时间格式hh:mm',
    description: '时间格式符合24小时格式规范',
    regexShow: '^([01]?[0-9]|2[0-3]):[0-5][0-9]$',
  },
  {
    id: 'hexColor',
    name: '十六进制颜色',
    description: '',
    regexShow: '^#?([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$',
  },
];
